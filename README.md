...
- **Architetto di Mondi**: Genera schede sensoriali dettagliate per luoghi e villaggi.
- **Emporio dei Volti**: Genera PNG completi di psicologia, segreti e ganci per incontri.
...
2.  **Crea il file di ambiente:**
    ...
    ```env
    GEMINI_API_KEY_STORY="xxxx"    # Obbligatoria: Generazione storie e base per gli altri strumenti
    GEMINI_API_KEY_SUMMARY="yyyy"  # Obbligatoria: Riassunti e catalogazione
    GEMINI_API_KEY_SHOPS="zzzz"    # Opzionale: Gestione botteghe (se omessa usa STORY)
    GEMINI_API_KEY_WORLD="wwww"    # Opzionale: Architetto di Mondi e PNG (se omessa usa STORY)
    ```
    *Puoi usare la stessa chiave per tutte le variabili se preferisci.*
