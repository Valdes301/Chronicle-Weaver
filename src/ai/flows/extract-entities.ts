...
const CharacterEventSchema = z.object({
  ...
  event: z.string().describe("Un breve riassunto di cosa ha fatto o cosa gli è successo in questa sessione."),
...
const prompt = ai.definePrompt({
    ...
    prompt: `Sei un esperto analista di sessioni di D&D 5e. Il tuo compito è estrarre OGNI elemento TANGIBILE e FISICO incontrato e analizzare il ruolo dei PERSONAGGI nella scena.
...
- Per ogni personaggio già noto (lista fornita): descrivi brevemente cosa ha fatto o cosa gli è successo.
- Se appare un nuovo PNG significativo: estrai il nome e un breve riassunto del suo ruolo.
- Sii conciso (max 15-20 parole per evento).
