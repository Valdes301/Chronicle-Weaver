...
import type { MagicItem, Monster, Spell, Weapon, Armor, Skill, CampaignWithRelations, Session, Campaign, PlayerCharacter, LetterPreset, Shop, WorldLocation, Npc, Combat } from '@/lib/types';
...
async function getCampaignsData(activeCampaignId?: string): Promise<{ 
    ...
    try {
        ...
        if (!campaigns || campaigns.length === 0) {
            return { campaigns: [], activeCampaign: null, sessions: [], magicItems: [], monsters: [], playerCharacters: [], customSpells: [], customSkills: [], possessedItems: [], letterPresets, shops: [], worldLocations: [], npcs: [], combats: [] };
        }
        ...
        return { campaigns, activeCampaign: activeCampaignData, sessions, magicItems, monsters, playerCharacters, customSpells, customSkills, possessedItems: possessedItems.map(i => i.itemName), letterPresets, shops, worldLocations, npcs, combats };
...
const uniqueByName = <T extends { name: string; source?: 'base' | 'created' }>(baseItems: T[], createdItems: T[]): T[] => {
    ...
    // Gli oggetti creati hanno la priorità
    for (const item of createdItems) {
        map.set(item.name.toLowerCase(), { ...item, source: 'created' as const });
    }
    // Gli oggetti base vengono aggiunti solo se non esiste già una versione creata
...
export default async function Page(props: { 
  ...
  const { campaigns, activeCampaign, sessions, magicItems: createdMagicItems, monsters: createdMonsters, playerCharacters, customSpells, customSkills, possessedItems, letterPresets, shops, worldLocations, npcs, combats } = await getCampaignsData(campaignId);
  ...
  // Mappatura robusta degli oggetti creati: ci assicuriamo che TUTTI i campi vengano passati
