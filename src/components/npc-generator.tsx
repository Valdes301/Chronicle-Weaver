        ...
        <div className="space-y-4">
          ...
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {savedNpcs.map((n) => {
              ...
              return (
                <Card key={n.id} className={cn("group hover:border-primary/50 transition-all flex flex-col", isViewingHistory && "col-span-full ring-2 ring-primary")}>
                  <CardHeader className="pb-3">
                    <div className="flex justify-between items-start">
                      <CardTitle className="font-headline text-xl text-primary leading-tight line-clamp-1">{n.name}</CardTitle>
                      <div className="flex gap-1">
                          <Button variant="ghost" size="icon" onClick={() => isViewingHistory ? setViewingHistory(null) : loadHistory(n)} className={cn("h-8 w-8", isViewingHistory ? "text-primary" : "text-muted-foreground")}>
                    ...
                    {isViewingHistory ? (
                        <div className="space-y-4 animate-in fade-in slide-in-from-top-2">
                            <div className="flex items-center gap-2 text-xs font-bold uppercase text-primary border-b pb-1">
                                <History className="h-3 w-3" /> Cronologia degli Eventi
                            </div>
                                    ...
                                    <div className="space-y-3">
                                        {historyData.map((ev, i) => (
                                            <div key={i} className="text-sm border-l-2 border-primary/20 pl-3 py-1">
                                                <div className="text-[10px] font-bold text-muted-foreground mb-1 uppercase">Sess. {ev.sessionNumber}: {ev.sessionTitle}</div>
                                                <p className="italic text-foreground/80 leading-relaxed">"{ev.eventDescription}"</p>
                                            </div>
                                        ))}
                                    </div>
                                ) : (
                                    <p className="text-xs text-muted-foreground italic text-center p-8">Nessun evento registrato per questo personaggio.</p>
                                )}
                            </ScrollArea>
                        </div>
                    ) : (
                  ...
                  <CardFooter className="pt-0 justify-end gap-2">
                      <Button variant="ghost" size="sm" className="text-[10px] h-7" onClick={() => setTempNpc(details)}>
                          <BookOpen className="mr-1 h-3 w-3" /> Vedi Scheda Psicologica
                      </Button>
                  </CardFooter>
                </Card>
              );
