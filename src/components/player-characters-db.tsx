...
import { Plus, Trash2, Pencil, ImagePlus, Loader2, History, User2, Star, Sparkles, Target, Brain, Footprints, Fingerprint, ChevronDown, BookOpen, ScrollText, Heart, Shield } from 'lucide-react';
...
// Schema più flessibile per gestire stringhe vuote negli input numerici
...
const CharacterFormDialog = ({ character, onSave, onDelete, trigger, campaignId }: { character?: PlayerCharacter, onSave: (data: CharacterFormData & { campaignId: string }) => Promise<boolean>, onDelete?: (id: string) => void, trigger: React.ReactNode, campaignId: string }) => {
    ...
    // Usiamo un ID statico per il modulo per evitare problemi di lookup del pulsante submit
    ...
    const onInvalid = (errors: any) => {
        console.error("PG Form Validation Errors:", errors);
        toast({
            variant: 'destructive',
            title: "Dati non validi",
            description: "Alcuni campi numerici o obbligatori contengono errori.",
        });
    };
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="sm:max-w-4xl max-h-[95vh] overflow-hidden flex flex-col p-0 outline-none border-primary/20">
                ...
                <div className="flex-1 overflow-y-auto px-6 py-4">
                    <Form {...form}>
                        <form id={formId} onSubmit={form.handleSubmit(handleActualSubmit, onInvalid)} className="space-y-6">
                            <div className="flex flex-col md:flex-row gap-6">
                                <div className="w-full md:w-48 space-y-2 shrink-0 flex flex-col items-center">
                                    ...
                                    <Button variant="link" type="button" size="sm" className="p-0 h-auto text-[10px]" onClick={() => form.setValue('imageUrl', '')}>Rimuovi</Button>
                                </div>
                                        ...
                                        <div className="flex-1 min-w-0 flex items-center gap-2">
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex flex-wrap gap-1 mt-1">
                                                    <Badge variant="outline" className="text-[8px] h-3.5 px-1">{char.race}</Badge>
                                                    <Badge variant="secondary" className="text-[8px] h-3.5 px-1 uppercase">{char.class}</Badge>
                                                    {char.archetype && (
                                                        <Badge variant="secondary" className="text-[8px] h-3.5 px-1 uppercase bg-primary/20 text-primary border-primary/30">
                                                            {char.archetype}
                                                        </Badge>
                                                    )}
                                                    <Badge variant="outline" className="text-[8px] h-3.5 px-1">Liv. {char.level}</Badge>
                                                </div>
                                            </div>
                                    ...
                                    <ScrollArea className="h-48 rounded-lg bg-background/50 border p-3">
                                        {isLoadingHistory ? (
                                            ...
                                            historyData.map((ev, i) => (
                                                <div key={i} className="text-[11px] border-l-2 border-primary/30 pl-3 py-1 mb-3 last:mb-0 text-left">
