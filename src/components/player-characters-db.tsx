...
import { Users, Plus, Trash2, Pencil, Heart, Shield, ImagePlus, Loader2, History, ScrollText } from 'lucide-react';
...
export function PlayerCharactersDb({ campaignId, initialCharacters, oldData, skills, onSave, onDelete }: PlayerCharactersDbProps) {
    ...
    const loadHistory = async (pc: PlayerCharacter) => {
        setIsLoadingHistory(true);
        setViewingHistory(pc.id);
        try {
            const res = await actions.getCharacterHistory(pc.id);
        ...
        <div className="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
            <div>
                ...
                <p className="text-muted-foreground">
                  Gestisci le schede dei tuoi personaggi. L'IA terrà conto di queste informazioni e aggiornerà la loro cronologia personale dopo ogni sessione.
                </p>
            </div>
        ...
        {initialCharacters.length > 0 ? (
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {initialCharacters.map(char => {
                    const isViewingHistory = viewingHistory === char.id;
                    return (
                        <Card key={char.id} className={cn("flex flex-col overflow-hidden group hover:border-primary/50 transition-all", isViewingHistory && "col-span-full ring-2 ring-primary")}>
                            <div className="relative h-48 bg-muted border-b">
                                ...
                                <div className="absolute top-2 right-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                    <Button variant="secondary" size="icon" className={cn("h-8 w-8 shadow-md", isViewingHistory && "bg-primary text-primary-foreground")} onClick={() => isViewingHistory ? setViewingHistory(null) : loadHistory(char)} title="Cronologia Personaggio">
                            ...
                            <CardContent className="text-sm text-muted-foreground flex-grow">
                               {isViewingHistory ? (
                                   <div className="space-y-4 animate-in fade-in slide-in-from-top-2 py-2">
                                       <div className="flex items-center gap-2 text-xs font-bold uppercase text-primary border-b pb-1">
                                           <ScrollText className="h-3 w-3" /> Cronaca delle Gesta
                                       </div>
                                               ...
                                               <div className="space-y-4">
                                                   {historyData.map((ev, i) => (
                                                       <div key={i} className="text-sm border-l-2 border-primary/20 pl-4 py-1">
                                                           <div className="text-[10px] font-bold text-muted-foreground mb-1 uppercase tracking-tighter">Sess. {ev.sessionNumber}: {ev.sessionTitle}</div>
                                                           <p className="text-foreground leading-relaxed">"{ev.eventDescription}"</p>
                                                       </div>
                                                   ))}
                                               </div>
                                           ) : (
                                               <p className="text-xs text-muted-foreground italic text-center p-8">Il personaggio non ha ancora compiuto gesta memorabili registrate.</p>
                                           )}
                                       </ScrollArea>
                                   </div>
                               ) : (
