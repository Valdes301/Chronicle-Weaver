                        ...
                        {letters.slice(0, layout === 'A4' ? 1 : 2).map((letter, idx) => (
                            <div key={idx} className="p-4 border rounded-xl space-y-4 bg-muted/30">
                                ...
                                <div className="grid grid-cols-3 gap-2">
                                    ...
                                    <Button variant="outline" size="sm" asChild><label htmlFor={`bg-${idx}`} className="cursor-pointer text-[10px]">Texture</label></Button>
                                    <Input type="file" accept="image/*" className="hidden" id={`logo-${idx}`} onChange={(e) => handleImageUpload(idx, 'logo', e.target.files?.[0] || null)} />
                                    <Button variant="outline" size="sm" asChild><label htmlFor={`logo-${idx}`} className="cursor-pointer text-[10px]">Stemma</label></Button>
                                    <Input type="file" accept="image/*" className="hidden" id={`seal-${idx}`} onChange={(e) => handleImageUpload(idx, 'seal', e.target.files?.[0] || null)} />
                                    <Button variant="outline" size="sm" asChild><label htmlFor={`seal-${idx}`} className="cursor-pointer text-[10px]">Sigillo</label></Button>
                                </div>
                            </div>
                        ))}
