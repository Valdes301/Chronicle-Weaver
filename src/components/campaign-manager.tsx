...
// Caricamento Dinamico dei componenti pesanti per velocizzare l'avvio sul Raspberry Pi
...
const ContentImporter = dynamic(() => import('./content-importer').then(m => m.ContentImporter), { ssr: false });
...
const LetterGenerator = dynamic(() => import('./letter-generator').then(m => m.LetterGenerator), { 
    ssr: false,
    loading: () => <div className="flex items-center justify-center p-12 text-muted-foreground italic">Caricamento strumenti tipografici...</div>
});
...
export function CampaignManager({
  ...
  const [activeView, setActiveView] = useState(initialView === 'nuova' ? 'storia' : (initialView || 'storia'));
  ...
  }, [activeCampaign, campaigns, router, isCreating]);
  ...
  const handleGenerateSession = async (prompt: string, modification?: { storyToModify: string, request: string }) => {
    ...
    try {
      ...
      const newSessionData: Omit<Session, 'id' | 'campaignId' | 'createdAt' | 'updatedAt' | 'is_read' | 'loot_scanned' | 'arcId' | 'is_archived'> = {
      ...
      setPendingSession({ ...newSessionData, id: `${Date.now()}`, campaignId: activeCampaign.id, arcId: activeCampaign.activeArc?.id || '', createdAt: new Date().toISOString(), updatedAt: new Date().toISOString(), is_read: false, loot_scanned: false, is_archived: false });
  ...
  const handleConfirmSession = async (sessionToConfirm: Session) => {
    ...
    setLoading(true);
    try {
        const { id, campaignId, createdAt, updatedAt, is_read, arcId, loot_scanned, is_archived, ...sessionData } = sessionToConfirm;
        const result = await actions.confirmSession(sessionData, activeCampaign.id);
        if (result.success) {
            toast({ title: "Scena Aggiunta!", description: `La scena "${sessionToConfirm.title}" è stata aggiunta alla cronologia.` });
            setPendingSession(null);
            router.refresh();
        } else {
  ...
  const handleImportSession = async (notes: string, title: string) => {
    ...
    try {
        ...
        if (result.success) {
            toast({ title: "Sessione Importata!", description: `La sessione è stata aggiunta alla tua campagna.` });
            router.refresh();
        } else {
  ...
  const handleUpdateSessionNumber = async (sessionId: string, num: number) => {
    const result = await actions.updateSessionNumber(sessionId, num);
    if (result.success) {
      toast({ title: 'Numero Aggiornato!' });
      router.refresh();
    } else {
      toast({ variant: "destructive", title: "Errore", description: result.error || "Impossibile aggiornare il numero." });
    }
  };
  ...
  const handleReorderSessions = async (orderedIds: string[]) => {
    if (!activeCampaign) return;
    const result = await actions.reorderSessions(orderedIds);
    if (result.success) {
  ...
  const handleSummarizeCampaign = async () => {
    ...
    try {
        ...
        if (result.success) {
            toast({ title: "Sommario Aggiornato!", description: "L'IA ha creato un nuovo riassunto della campagna." });
            router.refresh();
        } else {
  ...
  const handleClearSessionLoot = async (sessionId: string) => {
      const result = await actions.clearSessionLoot(sessionId);
      if (result.success) {
          toast({ title: 'Bottino Resettato', description: 'Ora puoi rieseguire la scansione.' });
          router.refresh();
      } else {
  ...
  const handleDownloadBackup = async () => {
    try {
      ...
      toast({ title: "Backup Scaricato", description: "Il backup della tua campagna è stato salvato." });
    } catch (error: any) {
      toast({ variant: "destructive", title: "Errore", description: error.message || "Impossibile creare il file di backup." });
    }
  };
  ...
  const handleRestoreBackup = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        if (result.success) {
            toast({ title: "Backup Ripristinato!", description: "I dati della tua campagna sono stati caricati con successo." });
            window.location.href = '/';
        } else {
            throw new Error(result.error || "File di backup non valido o corrotto.");
        }
      } catch (error: any) {
        toast({ variant: "destructive", title: "Errore di Ripristino", description: error.message });
        setLoading(false);
      } finally {
        if (event.target) event.target.value = "";
      }
    };
    reader.readAsText(file); // Fixed: using readAsText instead of readAsDataURL for JSON files
  };
    ...
    switch (activeView) {
        case 'storia':
            return <CampaignDashboard 
                        ...
                        onUpdateSessionNumber={handleUpdateSessionNumber}
                        ...
                        onClearSessionLoot={handleClearSessionLoot}
                    />;
        case 'biblioteca':
            return <ArchiveView campaignId={activeCampaign.id} />;
        case 'riepilogo-png':
            return <NpcSummary campaignId={activeCampaign.id} npcs={npcs} />;
        case 'abilità':
            return <SkillsDb 
                        skills={skills} 
        ...
        case 'mappe':
            return <MapGenerator />;
        case 'lettere':
            return <LetterGenerator presets={letterPresets} />;
        case 'botteghe':
            return <ShopManager campaign={activeCampaign} dbItems={allItems} savedShops={shops} />;
        case 'architetto':
            return <WorldArchitect campaign={activeCampaign} savedLocations={worldLocations} />;
        case 'anagrafe':
            return <NpcGenerator campaign={activeCampaign} savedNpcs={npcs} />;
        case 'combattimenti':
            return <CombatGenerator campaign={activeCampaign} savedCombats={combats} />;
        case 'tesori':
            return <TreasureGenerator campaign={activeCampaign} />;
        case 'importa':
            return <ContentImporter campaignId={activeCampaign.id} />;
        case 'manuale-importa':
            return <CatalogHandbook campaignId={activeCampaign.id} />;
        case 'crea-carte':
    ...
    <>
      ...
      {isCreating ? (
        <CampaignInitializer 
            ...
            onCancel={activeCampaign ? () => {
                setIsCreating(false);
                router.push(`/?campaignId=${activeCampaign.id}&view=storia`);
            } : undefined}
        />
      ) : activeCampaign ? (
        <SidebarProvider>
