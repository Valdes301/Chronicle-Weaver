...
const updateStmt = db.prepare(`UPDATE SystemPrompt SET content = ?, defaultContent = ? WHERE slug = ? AND (content NOT LIKE '%PG%' OR content NOT LIKE '%schede%')`);

defaultPrompts.forEach(p => {
    seedStmt.run(p.slug, p.title, p.content, p.content);
    // Forza l'aggiornamento se mancano i riferimenti chiave (per i database esistenti)
    if (p.slug === 'story-gen') {
        updateStmt.run(p.content, p.content, p.slug);
    }
});
