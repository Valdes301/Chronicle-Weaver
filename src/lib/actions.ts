...
import type { Session, CampaignWithRelations, Campaign, MagicItem, Monster, PlayerCharacter, Spell, Skill, LetterPreset, Shop, ShopItem, WorldLocation, LocationDetails, Npc, NpcDetails, Combat, CombatDetails, GenerateCombatInput, GenerateNpcInput, GenerateShopInput, GenerateLocationInput } from '@/lib/types';
...
export async function uploadGenericImage(imageData: string) {
    try {
        ...
        const base64Data = imageData.split(';base64,').pop();
        if (!base64Data) throw new Error('Dati base64 non trovati.');
        ...
        return actionResponse({ url: `/uploads/${filename}` });
    } catch (error: any) {
        return actionResponse(null, "Errore caricamento: " + error.message);
    }
}

// AI-related Actions

export async function analyzeContentAction(fileContent: string) {
    try {
...
export async function catalogHandbookAction(content: string, photoDataUri?: string) {
    try {
        const result = await catalogHandbookFlow({ content, photoDataUri });
        return actionResponse(result);
    } catch (error: any) {
        console.error("Error in catalogHandbookAction:", error);
        return actionResponse(null, `Errore dell'Importazione Intelligente: ${error.message}`);
    }
}
...
export async function migrateOldCharacters(campaignId: string, oldCharactersData: string) {
    try {
        ...
        const insert = db.prepare(
            `INSERT INTO PlayerCharacter (id, campaignId, name, class, archetype, strength, dexterity, constitution, intelligence, wisdom, charisma, background, createdAt, updatedAt, race, level, hitPoints, armorClass, skills, spells, pact, school, domain, traits, ideals, bonds, flaws, imageUrl) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
        );
        ...
        if (id) {
            ...
            db.prepare(
                `UPDATE PlayerCharacter SET 
                    ...
                    traits = ?, ideals = ?, bonds = ?, flaws = ?, imageUrl = ?,
                    updatedAt = ?
                ...
                data.traits, data.ideals, data.bonds, data.flaws, data.imageUrl,
                now, id, campaignId
            );
        } else {
            // Create
...
async function extractAndSaveEntities(storyText: string, campaignId: string) {
    ...
    const insertMagicItem = db.prepare(
        `INSERT OR IGNORE INTO MagicItem (id, name, type, rarity, attunement, description, campaignId, cost, damage, techType) 
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
    );
    ...
    db.transaction(() => {
        if (result.newMagicItems && result.newMagicItems.length > 0) {
            for (const item of result.newMagicItems) {
                // Infer techType for AI extracted items
                let techType = 'damage';
                const desc = item.description.toLowerCase();
                if (desc.includes('cura') || desc.includes('guarigione')) techType = 'cure';
                else if (item.type.toLowerCase().includes('armatura') || item.type.toLowerCase().includes('scudo')) techType = 'defense';
                else if (item.type.toLowerCase().includes('pozione')) techType = 'alchemy';

                insertMagicItem.run(randomUUID(), item.name, item.type, item.rarity, item.attunement ?? 'No', item.description, campaignId, item.cost, (item as any).damage, techType);
            }
        }
        if (result.newMonsters && result.newMonsters.length > 0) {
            for (const monster of result.newMonsters) {
...
export async function getBackupData() {
    try {
        ...
        const data = campaigns.map(campaign => {
            ...
            const possessedItems = db.prepare('SELECT itemName FROM PossessedItems WHERE campaignId = ?').all(campaign.id);
            const shops = db.prepare('SELECT * FROM Shop WHERE campaignId = ?').all(campaign.id);
            const worldLocations = db.prepare('SELECT * FROM WorldLocation WHERE campaignId = ?').all(campaign.id);
            const npcs = db.prepare('SELECT * FROM Npc WHERE campaignId = ?').all(campaign.id);
            const combats = db.prepare('SELECT * FROM Combat WHERE campaignId = ?').all(campaign.id);
            return { ...campaignData, sessions, magicItems, monsters, playerCharacters: playerCharactersData, customSpells, customSkills, possessedItems, shops, worldLocations, npcs, combats };
        });
...
export async function restoreBackupData(jsonData: string) {
    try {
        ...
        const restoreTransaction = db.transaction((campaignsToRestore: any[]) => {
            ...
            const magicItemStmt = db.prepare(`INSERT INTO MagicItem (id, name, type, rarity, attunement, description, cost, damage, techType, campaignId, imageUrl) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
            const monsterStmt = db.prepare(`INSERT INTO Monster (id, name, type, armorClass, hitPoints, challenge, description, campaignId, imageUrl) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
            const playerCharacterStmt = db.prepare(`INSERT INTO PlayerCharacter (id, campaignId, name, class, archetype, strength, dexterity, constitution, intelligence, wisdom, charisma, background, createdAt, updatedAt, race, level, hitPoints, armorClass, skills, spells, pact, school, domain, traits, ideals, bonds, flaws, imageUrl) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
            ...
            const shopStmt = db.prepare(`INSERT INTO Shop (id, campaignId, name, owner, description, inventory, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
            const worldLocationStmt = db.prepare(`INSERT INTO WorldLocation (id, campaignId, name, scale, style, atmosphere, details, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
            const npcStmt = db.prepare(`INSERT INTO Npc (id, campaignId, name, race, gender, age, status, alignment, details, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
            const combatStmt = db.prepare(`INSERT INTO Combat (id, campaignId, name, difficulty, details, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)`);
            ...
            for (const campaign of campaignsToRestore) {
                ...
                db.prepare('DELETE FROM Shop WHERE campaignId = ?').run(campaign.id);
                db.prepare('DELETE FROM WorldLocation WHERE campaignId = ?').run(campaign.id);
                db.prepare('DELETE FROM Npc WHERE campaignId = ?').run(campaign.id);
                db.prepare('DELETE FROM Combat WHERE campaignId = ?').run(campaign.id);
                ...
                if (campaign.magicItems?.length > 0) {
                    for (const i of campaign.magicItems) {
                        magicItemStmt.run(i.id || randomUUID(), i.name, i.type, i.rarity, i.attunement, i.description, i.cost, i.damage, i.techType || 'damage', campaign.id, i.imageUrl);
                    }
                }
                if (campaign.monsters?.length > 0) {
                    for (const m of campaign.monsters) {
                        monsterStmt.run(m.id || randomUUID(), m.name, m.type, m.armorClass, m.hitPoints, m.challenge, m.description, campaign.id, m.imageUrl);
                    }
                }
                ...
                if (campaign.shops?.length > 0) {
                    for (const sh of campaign.shops) {
                        shopStmt.run(sh.id || randomUUID(), campaign.id, sh.name, sh.owner, sh.description, sh.inventory, sh.createdAt || now, sh.updatedAt || sh.createdAt || now);
                    }
                }
                if (campaign.worldLocations?.length > 0) {
                    for (const wl of campaign.worldLocations) {
                        worldLocationStmt.run(wl.id || randomUUID(), campaign.id, wl.name, wl.scale, wl.style, wl.atmosphere, wl.details, wl.createdAt || now, wl.updatedAt || wl.createdAt || now);
                    }
                }
                if (campaign.npcs?.length > 0) {
                    for (const n of campaign.npcs) {
                        npcStmt.run(n.id || randomUUID(), campaign.id, n.name, n.race, n.gender, n.age, n.status, n.alignment, n.details, n.createdAt || now, n.updatedAt || n.createdAt || now);
                    }
                }
                if (campaign.combats?.length > 0) {
                    for (const c of campaign.combats) {
                        combatStmt.run(c.id || randomUUID(), campaign.id, c.name, c.difficulty, c.details, c.createdAt || now, c.updatedAt || c.createdAt || now);
                    }
                }
            }
        });
...
export async function saveMonster(monsterData: Partial<Monster> & { campaignId: string }) {
    try {
        ...
        if (id) {
            db.prepare(`
                UPDATE Monster SET name = ?, type = ?, armorClass = ?, hitPoints = ?, challenge = ?, description = ?, imageUrl = ? 
                WHERE id = ? AND campaignId = ?
            `).run(data.name, data.type, data.armorClass, data.hitPoints, data.challenge, data.description, data.imageUrl, id, campaignId);
        } else {
            db.prepare(`
                INSERT INTO Monster (id, name, type, armorClass, hitPoints, challenge, description, campaignId, imageUrl)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ...
            `).run(randomUUID(), data.name, data.type, data.armorClass, data.hitPoints, data.challenge, data.description, campaignId, data.imageUrl);
        }
        return actionResponse({ success: true });
    } catch (error: any) {
...
export async function saveMagicItem(itemData: Partial<MagicItem> & { campaignId: string }) {
    try {
        ...
        if (id) {
            db.prepare(`
                UPDATE MagicItem SET name = ?, type = ?, rarity = ?, attunement = ?, description = ?, cost = ?, damage = ?, techType = ?, imageUrl = ?
                WHERE id = ? AND campaignId = ?
            `).run(data.name, data.type, data.rarity, data.attunement ?? 'No', data.description, data.cost, data.damage, data.techType || 'damage', data.imageUrl, id, campaignId);
        } else {
            db.prepare(`
                INSERT INTO MagicItem (id, name, type, rarity, attunement, description, cost, damage, techType, campaignId, imageUrl)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ON CONFLICT(name, campaignId) DO UPDATE SET
                    type = excluded.type,
                    ...
                    techType = excluded.techType,
                    imageUrl = excluded.imageUrl
            `).run(randomUUID(), data.name, data.type, data.rarity, data.attunement ?? 'No', data.description, data.cost, data.damage, data.techType || 'damage', campaignId, data.imageUrl);
        }
        return actionResponse({ success: true });
    } catch (error: any) {
...
export async function saveLetterPreset(name: string, settings: string) {
    try {
        const id = randomUUID();
        db.prepare('INSERT INTO LetterPreset (id, name, settings) VALUES (?, ?, ?)').run(id, name, settings);
        return actionResponse({ success: true });
    } catch (error: any) {
        return actionResponse(null, "Impossibile salvare il preset: " + error.message);
    }
}
...
export async function deleteLetterPreset(id: string) {
    try {
        ...
        return actionResponse(null, "Impossibile eliminare il preset: " + error.message);
    }
}

export async function getLetterPresets() {
    try {
        const presets = db.prepare('SELECT * FROM LetterPreset ORDER BY createdAt DESC').all() as LetterPreset[];
        return actionResponse(presets);
    } catch (error: any) {
        return actionResponse(null, error.message);
    }
}
...
export async function generateShopAction(input: GenerateShopInput) {
    try {
        const result = await generateShopFlow(input);
        return actionResponse(result);
    } catch (error: any) {
        return actionResponse(null, error.message);
    }
...
export async function saveShop(shopData: Omit<Shop, 'id' | 'createdAt' | 'updatedAt'>) {
    try {
        const id = randomUUID();
        const now = new Date().toISOString();
        db.prepare(`
            INSERT INTO Shop (id, campaignId, name, owner, description, inventory, createdAt, updatedAt)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)
        `).run(id, shopData.campaignId, shopData.name, shopData.owner, shopData.description, shopData.inventory, now, now);
...
export async function getShops(campaignId: string) {
    try {
        const shops = db.prepare('SELECT * FROM Shop WHERE campaignId = ? ORDER BY updatedAt DESC').all(campaignId) as Shop[];
        return actionResponse(shops);
    } catch (error: any) {
        return actionResponse(null, error.message);
    }
}
...
export async function generateLocationAction(input: GenerateLocationInput) {
    try {
        const result = await generateLocationFlow(input);
        return actionResponse(result);
    } catch (error: any) {
        return actionResponse(null, error.message);
    }
...
export async function saveWorldLocation(locationData: Omit<WorldLocation, 'id' | 'createdAt' | 'updatedAt'>) {
    try {
        const id = randomUUID();
        const now = new Date().toISOString();
        db.prepare(`
            INSERT INTO WorldLocation (id, campaignId, name, scale, style, atmosphere, details, createdAt, updatedAt)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(id, locationData.campaignId, locationData.name, locationData.scale, locationData.style, locationData.atmosphere, locationData.details, now, now);
        return actionResponse({ id });
    } catch (error: any) {
        return actionResponse(null, error.message);
    }
...
export async function generateNpcAction(input: GenerateNpcInput) {
    try {
        const result = await generateNpcFlow(input);
        return actionResponse(result);
    } catch (error: any) {
        return actionResponse(null, error.message);
    }
...
export async function saveNpc(npcData: Omit<Npc, 'id' | 'createdAt' | 'updatedAt'>) {
    try {
        const id = randomUUID();
        const now = new Date().toISOString();
        db.prepare(`
            INSERT INTO Npc (id, campaignId, name, race, gender, age, status, alignment, details, createdAt, updatedAt)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        `).run(id, npcData.campaignId, npcData.name, npcData.race, npcData.gender, npcData.age, npcData.status, npcData.alignment, npcData.details, now, now);
        return actionResponse({ id });
    } catch (error: any) {
        return actionResponse(null, error.message);
    }
...
export async function generateCombatAction(input: GenerateCombatInput) {
    try {
        const result = await generateCombatFlow(input);
        return actionResponse(result);
    } catch (error: any) {
        return actionResponse(null, error.message);
    }
...
export async function saveCombat(combatData: Omit<Combat, 'id' | 'createdAt' | 'updatedAt'>) {
    try {
        const id = randomUUID();
        const now = new Date().toISOString();
        db.prepare(`
            INSERT INTO Combat (id, campaignId, name, difficulty, details, createdAt, updatedAt)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        `).run(id, combatData.campaignId, combatData.name, combatData.difficulty, combatData.details, now, now);
        return actionResponse({ id });
    } catch (error: any) {
        return actionResponse(null, error.message);
    }
...
export async function bulkImportAction(campaignId: string, data: any) {
    try {
        const { items, monsters, spells, skills } = data;
        let count = 0;

        db.transaction(() => {
            if (items && Array.isArray(items)) {
                const stmt = db.prepare(`INSERT OR IGNORE INTO MagicItem (id, name, type, rarity, attunement, description, cost, damage, techType, campaignId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
                for (const i of items) {
                    // Smart techType inference
                    let techType = i.techType;
                    if (!techType) {
                        const desc = (i.description || '').toLowerCase();
                        const type = (i.type || '').toLowerCase();
                        if (desc.includes('cura') || desc.includes('guarigione')) techType = 'cure';
                        else if (type.includes('armatura') || type.includes('scudo')) techType = 'defense';
                        else if (type.includes('pozione')) techType = 'alchemy';
                        else techType = 'damage';
                    }
                    stmt.run(randomUUID(), i.name, i.type, i.rarity, i.attunement || 'No', i.description, i.cost, i.damage, techType, campaignId);
                    count++;
                }
            }
            if (monsters && Array.isArray(monsters)) {
                const stmt = db.prepare(`INSERT OR IGNORE INTO Monster (id, name, type, armorClass, hitPoints, challenge, description, campaignId) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`);
                for (const m of monsters) {
                    stmt.run(randomUUID(), m.name, m.type, m.armorClass, m.hitPoints, m.challenge, m.description, campaignId);
                    count++;
                }
            }
            if (spells && Array.isArray(spells)) {
                const stmt = db.prepare(`INSERT OR IGNORE INTO CustomSpell (id, name, level, school, casting_time, range, components, duration, description, classes, campaignId) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
                for (const s of spells) {
                    stmt.run(randomUUID(), s.name, s.level, s.school, s.casting_time, s.range, s.components, s.duration, s.description, s.classes, campaignId);
                    count++;
                }
            }
            if (skills && Array.isArray(skills)) {
                const stmt = db.prepare(`INSERT OR IGNORE INTO CustomSkill (id, name, ability, description, campaignId) VALUES (?, ?, ?, ?, ?)`);
                for (const sk of skills) {
                    stmt.run(sk.id || randomUUID(), sk.name, sk.ability, sk.description, campaignId);
                    count++;
                }
            }
        })();
