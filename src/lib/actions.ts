...
export async function scanSessionForLoot(sessionId: string) {
    try {
        ...
        db.transaction(() => {
            ...
            const hasMagicItemDates = magicItemCols.includes('createdat');

            for (const item of entities.newMagicItems) {
                if (hasMagicItemDates) {
                    db.prepare(`INSERT INTO MagicItem (id, name, type, rarity, attunement, description, cost, damage, techType, imageUrl, campaignId, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(name, campaignId) DO UPDATE SET type=excluded.type, rarity=excluded.rarity, attunement=excluded.attunement, description=excluded.description, cost=excluded.cost, damage=excluded.damage, techType=excluded.techType, imageUrl=excluded.imageUrl, updatedAt=excluded.updatedAt`).run(randomUUID(), item.name, item.type, item.rarity, item.attunement || 'No', item.description, item.cost || 'N/D', item.damage || '', 'damage', null, session.campaignId, now, now);
                } else {
            ...
            const hasRewardDates = rewardCols.includes('createdat');

            for (const r of entities.rewards) {
                if (hasRewardDates) {
                    db.prepare(`INSERT INTO Reward (id, campaignId, sessionId, name, description, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)`).run(randomUUID(), session.campaignId, sessionId, r.name, r.description, now, now);
                } else {
            ...
            entities.characterEvents.forEach((event, idx) => {
                ...
                if (!charId) {
                    ...
                    if (!charId && event.isNew) {
                        ...
                        const hasNpcDates = npcCols.includes('createdat');

                        if (hasNpcDates) {
                            db.prepare(`INSERT INTO Npc (id, campaignId, name, race, gender, age, status, alignment, details, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(charId, session.campaignId, event.name, 'Sconosciuta', 'Maschio', 'Adulto', 'Normale', 'Neutrale', JSON.stringify(d), now, now);
                        } else {
                            db.prepare(`INSERT INTO Npc (id, campaignId, name, race, gender, age, status, alignment, details) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(charId, session.campaignId, event.name, 'Sconosciuta', 'Maschio', 'Adulto', 'Normale', 'Neutrale', JSON.stringify(d));
                        }
                    }
                }
                if (charId) {
                    const cols = ["id", "characterId", "characterType", "sessionId", "campaignId", "eventDescription"];
                    const vals = [randomUUID(), charId, charType, sessionId, session.campaignId, event.event];
    ...
    try {
        ...
        db.transaction(() => {
            ...
            result.events.forEach((ev, idx) => {
                ...
                if (sess) db.prepare(`INSERT INTO CharacterEvent (id, characterId, characterType, sessionId, campaignId, eventDescription, order_index, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(randomUUID(), npcId, 'npc', sess.id, campaignId, ev.eventDescription, idx, now);
            });
...
export async function saveMonster(monster: Partial<Monster> & { campaignId: string }) {
    try {
        ...
        if (hasDates) {
            db.prepare(`INSERT INTO Monster (id, name, type, armorClass, hitPoints, challenge, description, imageUrl, campaignId, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(name, campaignId) DO UPDATE SET type=excluded.type, armorClass=excluded.armorClass, hitPoints=excluded.hitPoints, challenge=excluded.challenge, description=excluded.description, imageUrl=excluded.imageUrl, updatedAt=excluded.updatedAt`).run(id, monster.name, monster.type, monster.armorClass, monster.hitPoints, monster.challenge, monster.description, monster.imageUrl || null, monster.campaignId, monster.createdAt || now, now);
        } else {
...
export async function saveSkill(skill: Partial<Skill> & { campaignId: string }) {
    try {
        ...
        if (hasDates) {
            db.prepare(`INSERT INTO CustomSkill (id, name, ability, description, campaignId, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(name, campaignId) DO UPDATE SET ability=excluded.ability, description=excluded.description, updatedAt=excluded.updatedAt`).run(id, skill.name, skill.ability, skill.description, skill.campaignId, skill.createdAt || now, now);
        } else {
    ...
    try {
        ...
        db.transaction(() => {
            ...
            if (data.items) {
                ...
                const hasDates = cols.includes('createdat');
                const sql = hasDates 
                    ? `INSERT OR IGNORE INTO MagicItem (id, campaignId, name, type, rarity, attunement, description, cost, damage, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
                    : `INSERT OR IGNORE INTO MagicItem (id, campaignId, name, type, rarity, attunement, description, cost, damage) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`;
                
                const s = db.prepare(sql);
                for (const it of data.items) {
                    const vals = [randomUUID(), campaignId, it.name, it.type || 'Oggetto', it.rarity || 'Comune', it.attunement || 'No', it.description || '', it.cost || 'N/D', it.damage || ''];
                    if (hasDates) vals.push(now, now);
                    s.run(...vals);
                    count++;
                }
            }
            ...
            if (data.monsters) {
                ...
                const hasDates = cols.includes('createdat');
                const sql = hasDates
                    ? `INSERT OR IGNORE INTO Monster (id, campaignId, name, type, armorClass, hitPoints, challenge, description, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
                    : `INSERT OR IGNORE INTO Monster (id, campaignId, name, type, armorClass, hitPoints, challenge, description) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`;

                const s = db.prepare(sql);
                for (const m of data.monsters) {
                    const vals = [randomUUID(), campaignId, m.name, m.type || 'Mostro', m.armorClass || '10', m.hitPoints || '10', m.challenge || '0', m.description || ''];
                    if (hasDates) vals.push(now, now);
                    s.run(...vals);
                    count++;
                }
            ...
            if (data.spells) {
                ...
                const hasDates = cols.includes('createdat');
                const sql = hasDates
                    ? `INSERT OR IGNORE INTO CustomSpell (id, campaignId, name, level, school, casting_time, range, components, duration, description, classes, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
                    : `INSERT OR IGNORE INTO CustomSpell (id, campaignId, name, level, school, casting_time, range, components, duration, description, classes) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;

                const s = db.prepare(sql);
                for (const spell of data.spells) {
                    const vals = [randomUUID(), campaignId, spell.name, spell.level || '0', spell.school || 'Univ', spell.casting_time || '1 az', spell.range || 'Contatto', spell.components || 'V, S', spell.duration || 'Ist', spell.description || '', spell.classes || ''];
                    if (hasDates) vals.push(now, now);
                    s.run(...vals);
                    count++;
                }
            ...
            if (data.skills) {
                ...
                const hasDates = cols.includes('createdat');
                const sql = hasDates
                    ? `INSERT OR IGNORE INTO CustomSkill (id, campaignId, name, ability, description, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?)`
                    : `INSERT OR IGNORE INTO CustomSkill (id, campaignId, name, ability, description) VALUES (?, ?, ?, ?, ?)`;

                const s = db.prepare(sql);
                for (const sk of data.skills) {
                    const vals = [randomUUID(), campaignId, sk.name, sk.ability || 'Varia', sk.description || ''];
                    if (hasDates) vals.push(now, now);
                    s.run(...vals);
                    count++;
                }
...
export async function saveHomebrewRule(data: Partial<HomebrewRule> & { campaignId: string }) {
    try {
        ...
        const now = new Date().toISOString();
        db.prepare(`INSERT INTO HomebrewRule (id, campaignId, title, content, category, isActive, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET title=excluded.title, content=excluded.content, category=excluded.category, isActive=excluded.isActive, updatedAt=excluded.updatedAt`).run(id, data.campaignId, data.title, data.content, data.category || 'Generale', data.isActive ? 1 : 0, data.createdAt || now, now);
        return actionResponse({ id });
    } catch (e: any) { return actionResponse(null, e.message); }
}
...
export async function quickSaveRewardAction(campaignId: string, name: string, description: string) {
    try {
        const now = new Date().toISOString();
        db.prepare(`INSERT INTO Reward (id, campaignId, sessionId, name, description, createdAt, updatedAt) VALUES (?, ?, 'manual', ?, ?, ?, ?)`).run(randomUUID(), campaignId, name, description, now, now);
        return actionResponse({ success: true });
    } catch (e: any) { return actionResponse(null, e.message); }
}
...
export async function saveCombat(data: any) {
    try {
        const id = data.id || randomUUID();
        const now = new Date().toISOString();
        db.prepare(`INSERT INTO Combat (id, campaignId, name, difficulty, details, createdAt, updatedAt) VALUES (?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET name=excluded.name, difficulty=excluded.difficulty, details=excluded.details, updatedAt=excluded.updatedAt`).run(id, data.campaignId, data.name, data.difficulty, data.details, data.createdAt || now, now);
        return actionResponse({ id });
    } catch (e: any) { return actionResponse(null, e.message); }
}
